package io.github.binaryoverload.permissions;
...
public class PerGuildPermissions {
    ...
    private static final CookieBot fb = CookieBot.getInstance();
    ...
    public boolean hasPermission(Member user, String permission) {
        ...
        if (isContributor(user.getUser()) && CookieBot.getInstance().isTestBot())
            return true;
    ...
    private static boolean checkOfficialGuildForRole(net.dv8tion.jda.core.entities.User user, long roleId) {
        if (!CookieBot.getInstance().isReady() || fb.getOfficialGuild() == null) return false;
    ...
    public void createDefaultGroup() {
        ...
        for (Command command : CookieBot.getInstance().getCommands()) {
