...
public class FutureAction {
    ...
    public void execute() {
        GuildWrapper gw = CookieBotManager.getInstance().getGuild(String.valueOf(guildId));
        switch (action) {
            case TEMP_MUTE:
            ...
            case TEMP_BAN:
                ...
                break;
            case REMINDER:
                if (CookieBot.getInstance().getChannelById(channelId) != null)
                    CookieBot.getInstance().getChannelById(channelId).sendMessage(CookieBot.getInstance()
