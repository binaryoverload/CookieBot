...
public class InfoCommand implements Command {
    ...
    static {
        if (new File("CookieBot" + File.separator).exists()) {
            try {
                ProcessBuilder p = new ProcessBuilder("git", "log", "--pretty=format:'%h'", "-n", "1");
                p.directory(new File("CookieBot" + File.separator));
                Process pr = p.start();
                pr.waitFor();
                if (pr.exitValue() == 0) {
                ...
                CookieBot.LOGGER.error("Could not compare git revisions!", e1);
            }
        }
    }
    ...
    public void onCommand(User sender, Guild guild, TextChannel channel, Message message, String[] args, Member member) {
        if (args.length == 0) {
            ...
            bld.setDescription("CookieBot v" + CookieBot.getInstance().getVersion() + " info")
                    .setColor(Color.CYAN);
    ...
    public enum Content implements MultiSelectionContent<String, String, Boolean> {
        ...
        SERVERS("Servers", () -> String.valueOf(CookieBot.getInstance().getGuilds().size())),
        VERSION("Version", CookieBot.getInstance().getVersion()),
        JDA_VERSION("JDA version", JDAInfo.VERSION),
        GIT("Git Revision", (git != null ? git : "Unknown")),
        SOURCE("Source", "[`GitHub`](https://github.com/CookieBot/CookieBot)"),
        INVITE("Invite", String.format("[`Invite`](%s)", CookieBot.getInstance().getInvite())),
        EMPTY("\u200B", "\u200B", false),
        SUPPORT_SERVER("Support Server", "[`Discord`](https://flarebot.stream/support-server)"),
        WEBSITE("Website", "[`CookieBot`](http://flarebot.stream/)"),
