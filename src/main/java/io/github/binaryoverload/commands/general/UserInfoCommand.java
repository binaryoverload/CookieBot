...
public class UserInfoCommand implements Command {
    ...
    public void onCommand(User sender, Guild guild, TextChannel channel, Message message, String[] args, Member member) {
        ...
        if (channel.getGuild().getMember(user) == null) {
            Guild memberGuild = CookieBot.getInstance().getGuilds().stream().filter(g -> g.getMemberById(user.getId()) != null)
        ...
        PlayerCache cache = cookieBot.getPlayerCache(id);
        channel.sendMessage(MessageUtils.getEmbed(sender)
                ...
                .addField("General Info",
                        "Servers: " + CookieBot.getInstance().getGuilds().stream()
