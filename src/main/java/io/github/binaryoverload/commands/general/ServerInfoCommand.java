...
public class ServerInfoCommand implements Command {
    ...
    public void onCommand(User sender, Guild guild, TextChannel channel, Message message, String[] args, Member member) {
        if (args.length == 0) {
            ...
            Guild targetGuild = CookieBot.getInstance().getGuildById(GeneralUtils.getLong(args[0], -1));
            if (targetGuild != null) {
    ...
    private void sendGuildInfo(Guild guild, TextChannel channel) {
        ...
        String afk = guild.getAfkChannel() == null ? "" :
                "**AFK:**\n" +
                        ...
                        "\n" +
                        "Timeout: " +
                        CookieBot.getInstance().formatTime(guild.getAfkTimeout().getSeconds(), TimeUnit.SECONDS, true, false);
