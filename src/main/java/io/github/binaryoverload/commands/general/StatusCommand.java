...
public class StatusCommand implements Command {
    ...
    public void onCommand(User sender, Guild guild, TextChannel channel, Message message, String[] args, Member member) {
        CookieBot fb = CookieBot.getInstance();
        ...
        if (deadShard > 5)
            sb.append(" SEVERE: We have quite a few dead shards! Please report this on the [Support Server](")
                    .append(CookieBot.INVITE_URL).append(")").append("\n");
        ...
        sb.append(String.format("CookieBot Version: %s\n" +
                        "JDA Version: %s\n" +
                        "Current Shard: %s\n" +
                        "* Average Ping: %s\n" +
                        "* Ping By Shard: %s\n" +
        ...
        channel.sendMessage("**CookieBot's Status**\n```prolog\n" + sb.toString() + "\n```").queue();
    }
