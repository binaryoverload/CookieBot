...
import io.github.binaryoverload.scheduler.FlareBotTask;
...
public class UpdateCommand implements Command {
    ...
    private CookieBot cookieBot = CookieBot.getInstance();
            ...
            if (args.length == 0) {
                ...
                if (args[0].equalsIgnoreCase("force")) {
                    ...
                    channel.sendMessage("I will now update to the latest version when no channels are playing music!")
                            .queue();
                    if (cookieBot.getConnectedVoiceChannels() == 0) {
                        update(true, channel);
                    } else {
    ...
    /**
     * Update to the newest version of CookieBot!
     *
    ...
    public static void update(boolean force, TextChannel channel) {
        if (force) {
            doTheUpdate(channel, "latest", CookieBot.getInstance().getVersion());
            return;
        }
        try {
            URL url = new URL("https://raw.githubusercontent.com/CookieBot/CookieBot/master/pom.xml");
            ...
            while (true) {
                ...
                if (line != null && (line.contains("<version>") && line.contains("</version>"))) {
                    String latestVersion = line.replace("<version>", "").replace("</version>", "").replaceAll(" ", "")
                            .replaceAll("\t", "");
                    String currentVersion = CookieBot.getInstance().getVersion();
