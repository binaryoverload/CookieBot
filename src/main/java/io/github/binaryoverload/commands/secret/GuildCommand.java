...
import io.github.binaryoverload.util.MessageType;
...
public class GuildCommand implements Command {
        ...
        if (args.length == 0) {
            ...
            if (args[0].equalsIgnoreCase("block")) {
                if (args.length == 1) {
                    ...
                    handleBlock(channel, args[1], CookieBot.getMessage(args, 2));
                }
                ...
                if (args.length == 2) {
                    if (CookieBot.getInstance().getGuildById(args[1]) == null) {
                        MessageUtils.sendErrorMessage("That guild ID is not valid!", channel);
                        return;
                    }
                    wrapper = CookieBotManager.getInstance().getGuild(args[1]);
                }
                ...
                if (args.length == 1) {
                    ...
                    GuildWrapper guildWrapper = CookieBotManager.getInstance().getGuild(args[1]);
                ...
                if (wrapper == null) {
                    ...
                    try {
                        ...
                        out.println(CookieBot.GSON.toJson(wrapper));
                        out.close();
                    } catch (FileNotFoundException e) {
                        CookieBot.LOGGER.error("Failed to write data", e);
                    }
                ...
                GuildWrapper wrapper = guild;
                if (args.length >= 2) {
                    wrapper = CookieBotManager.getInstance().getGuild(CookieBot.getInstance().getGuildById(args[1]).getId());
                }
                if (wrapper == null) {
                    MessageUtils.sendErrorMessage("Invalid guild ID!", channel);
                    return;
                }
                CookieBotManager.getInstance().saveGuild(wrapper.getGuildId(), wrapper, -1);
    ...
    private void handleBlock(TextChannel channel, String guildId, String reason) {
        Guild guild1 = CookieBot.getInstance().getGuildById(guildId);
        if (guild1 == null) {
            MessageUtils.sendErrorMessage("That guild ID is not valid!", channel);
            return;
        }
        if (CookieBotManager.getInstance().getGuild(guild1.getId()).isBlocked()) {
            MessageUtils.sendErrorMessage("Guild already blocked!", channel);
            return;
        }
        CookieBotManager.getInstance().getGuild(guild1.getId()).addBlocked(reason);
    ...
    private void handleUnblock(TextChannel channel, String guildId) {
        ...
        CookieBotManager.getInstance().getGuild(guild1.getId()).revokeBlock();
