        ...
        if (event.getMarker() != Markers.NO_ANNOUNCE
            ...
            EXECUTOR.submit(() -> {
                ...
                if (event.getLevel() == Level.WARN) {
                    ...
                    MessageUtils.sendWarningMessage(finalMsg, CookieBot.getInstance().getErrorLogChannel());
                    return;
                }
                if (throwable != null) {
                    if (event.getMarker() == Markers.TAG_DEVELOPER)
                        MessageUtils.sendFatalException(finalMsg, throwable, CookieBot.getInstance().getErrorLogChannel());
                    else
                        MessageUtils.sendException(finalMsg, throwable, CookieBot.getInstance().getErrorLogChannel());
                } else {
                    if (event.getMarker() == Markers.TAG_DEVELOPER)
                        MessageUtils.sendFatalErrorMessage(finalMsg, CookieBot.getInstance().getErrorLogChannel());
                    else
                        MessageUtils.sendErrorMessage(finalMsg, CookieBot.getInstance().getErrorLogChannel());
                }
            });
        }
        return FilterReply.NEUTRAL;
