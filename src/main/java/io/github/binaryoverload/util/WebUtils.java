...
public class WebUtils {
    ...
    private static final Callback defaultCallback = new Callback() {
        ...
        public void onFailure(Call call, IOException e) {
            CookieBot.LOGGER.error("Error for " + call.request().method() + " request to " + call.request().url(), e);
        }
        ...
        public void onResponse(Call call, Response response) throws IOException {
            response.close();
            CookieBot.LOGGER.debug("Reponse for " + call.request().method() + " request to " + call.request().url());
        }
    };
    ...
    public static Response post(Request.Builder builder) throws IOException {
        return CookieBot.getOkHttpClient().newCall(builder.build()).execute();
    }
    ...
    public static Response get(Request.Builder builder) throws IOException {
        return CookieBot.getOkHttpClient().newCall(builder.get().build()).execute();
    }

    public static void postAsync(Request.Builder builder) {
        CookieBot.getOkHttpClient().newCall(builder.build()).enqueue(defaultCallback);
    }
