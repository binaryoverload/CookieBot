...
public class MessageUtils {
    ...
    private static CookieBot cookieBot = CookieBot.getInstance();
    ...
    public static void sendFatalException(String s, Throwable e, TextChannel channel) {
        ...
        channel.sendMessage(new MessageBuilder().append(
                cookieBot.getOfficialGuild().getRoleById(Constants.DEVELOPER_ID).getAsMention())
                .setEmbed(getEmbed().setColor(Color.red).setDescription(s + "\n**Stack trace**: " + paste(trace))
                        .build()).build()).queue();
    }

    public static String paste(String trace) {
        if (cookieBot.getPasteKey() == null || cookieBot.getPasteKey().isEmpty()) {
            CookieBot.LOGGER.warn("Paste server key is missing! Pastes will not work!");
            return null;
        }
        try {
            Response response = WebUtils.post(new Request.Builder().url("https://paste.flarebot.stream/documents")
                    .addHeader("Authorization", cookieBot.getPasteKey()).post(RequestBody
            ...
            if (body != null) {
                ...
                CookieBot.LOGGER.error("Local instance of hastebin is down");
                return null;
            }
        } catch (IOException | JSONException e) {
            CookieBot.LOGGER.error("Could not make POST request to paste!", e);
            return null;
        }
    }
    ...
    public static EmbedBuilder getEmbed() {
        return new EmbedBuilder()
                .setAuthor("CookieBot", "https://github.com/CookieBot/CookieBot", cookieBot.getSelfUser()
    ...
    public static void sendMessage(MessageType type, EmbedBuilder builder, TextChannel channel, long autoDeleteDelay) {
        ...
        if(type == MessageType.ERROR)
            builder.setDescription(builder.build().getDescription() + "\n\nIf you need more support join our " +
                    "[Support Server](" + CookieBot.INVITE_URL + ")! Our staff can support on any issue you may have! "
                    + CookieBot.getInstance().getEmoteById(386550693294768129L).getAsMention());
